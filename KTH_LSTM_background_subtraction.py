...
model_name = "KTH_Data_background_subtraction"
...
trg_data_root = "KTH_Data_Preprocessed/"
...
#                         data_gray = np.delete(data,[1,2],2)
                        data_gray = data.astype('float32')/255.0
                        current_seg_temp.append(data_gray)
                        data_array_np[z,n-clip_start_index,:,:,:] = data_gray.reshape((120,160,1))
...
model.add(TimeDistributed(Conv2D(16, 3, strides=(2, 2), padding='valid')))
model.add(Activation('relu'))

model.add(TimeDistributed(Conv2D(64, 3, strides=(2, 2), padding='valid')))
model.add(Activation('relu'))
...
nb_epochs = 100 # I once achieved 77.5% accuracy with 100 epochs. Feel free to change

print ("Loading data")
# load training data
X_train, y_train = load_data_for_persons(trg_data_root, 1, 8, maxToAdd)

X_val, y_val = load_data_for_persons(trg_data_root, 9, 16, maxToAdd)
